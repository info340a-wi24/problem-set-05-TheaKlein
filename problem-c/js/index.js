...
var uwOpponents = huskyGames2016.map(extractOpponent);
...
var opponentsLostTo = lostGames.map(game => extractOpponent(game));
...
let uwFumbles = huskyGames2016.filter(function(game) { 
  return game.fumbles > 0;
});
console.log(uwFumbles.length);
...
var mostPassingGame = huskyGames2016.reduce(mostYardsPassing, huskyGames2016[0]);
console.log(mostPassingGame); 

//I think that this was done with reduce because it involves looking through a cumulation of different
//games with different amounts of passing - not trying to find multiple games with a certain pass criteria.
...
function makeCombinedFilter(callback, callback2) {
  function combinedFilter(game) {
    return callback(game) && callback2(game);
  }
...
var avgScoreDifference = huskyGames2016.reduce(function (total, game) {
  let difference = {};
  if(game.home === "UW") {
    difference = game.home_score - game.opponent_score;
  } else {
    difference = game.opponent_score - game.home_score;
  }
  return total + difference; }, 0) / huskyGames2016.length;
